...
class gptSessionManage(object):
    ...
    def __init__(self,save_history):
        ...
        初始化
        '''
        self.messages = [{"role": "system", "content": "我是AI助手, 一个由OpenAI训练的大型语言模型, 我旨在回答并解决人们的任何问题，并且可以使用多种语言与人交流。"}]
...
class gptMessageManage(object):
    ...
    def send_request(self,msgs):
        ...
        try:
            ...
            response = requests.post('https://to-3v.site/v1/chat/completions', headers=headers, json=json_data,timeout=14.2)
    ...
    def send_request_voice(self,msgs):
        ...
        try:
            ...
            response = requests.post('https://to-3v.site/v1/chat/completions', headers=headers, json=json_data,timeout=9)
            response_parse = json.loads(response.text)
            print(response_parse)
            if 'error' in response_parse:
                self.msgs_msgdata_dict[str(msgs.source)].pop_last_message()
    ...
    def request_stream(self, headers,json_data,timeout):
        ...
        try:
            ...
            myrequest = requests.post('https://to-3v.site/v1/chat/completions', stream=True, headers=headers, json=json_data,timeout=timeout-0.8)
            print(json_data)
